#!/usr/bin/env python3
"""Maintainer tool: assemble reviewed runtime updates or optional NVIDIA releases."""
...
def contained_target(root: Path, relative: str) -> Path:
    ...
    if candidate == resolved_root or resolved_root not in candidate.parents or resolved_root not in resolved.parents:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="kitsumate-runtime-") as temporary:
        staged = argparse.Namespace(**vars(args))
        staged.destination = Path(temporary)
        provision_payload(lock, staged)
        ...
        for source in files:
            ...
            if source.is_symlink():
                if target.is_symlink() and target.readlink() == source.readlink():
                    continue
                target.unlink(missing_ok=True)
                target.symlink_to(source.readlink())
            elif target.is_symlink() or not target.is_file() or sha256(source) != sha256(target):
                if target.is_symlink():
                    target.unlink()
                shutil.copyfile(source, target)
        for path in destination.rglob("*"):
            relative = path.relative_to(destination)
            selected = args.platform is None or relative.parts[0] in (args.platform, "Managed", "Licenses")
            if selected and (path.is_file() or path.is_symlink()) and path.suffix != ".meta" and relative not in expected:
                path.unlink()
    return 0


def provision_payload(lock: dict, args) -> None:
