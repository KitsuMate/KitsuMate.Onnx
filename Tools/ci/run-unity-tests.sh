...
: "${KITSUMATE_MODEL_CACHE:?Set a writable cache directory for checksum-pinned models.}"
...
python3 "$repository_root/Tools/ci/download-fixtures.py" \
  --lock "$fixture_lock" \
  --cache "$KITSUMATE_MODEL_CACHE" \
  --repository-root "$repository_root"

echo "Downloaded ONNX Runtime and the required CPU CI fixture set."
