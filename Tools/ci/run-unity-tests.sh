...
python3 "$repository_root/Tools/ci/validate-onnxruntime.py" \
  --lock "$repository_root/Dependencies/onnxruntime.lock.json" \
  --directory "$backend_plugins"
...
echo "Verified the bundled ONNX Runtime and downloaded the required CPU CI fixtures."
