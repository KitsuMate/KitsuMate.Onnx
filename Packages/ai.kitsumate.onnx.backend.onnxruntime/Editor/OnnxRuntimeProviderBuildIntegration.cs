...
namespace KitsuMate.Onnx.Editor
{
    ...
    public static class OnnxRuntimePluginSetup
    {
        ...
        public static void ConfigureWindowsImporters()
        {
            string root = "Packages/ai.kitsumate.onnx.backend.onnxruntime/Runtime/Plugins/Windows/x86_64";
            string packageDirectory = UnityEditor.PackageManager.PackageInfo.FindForAssembly(typeof(OnnxRuntimeBackend).Assembly).resolvedPath;
            string directory = Path.Combine(packageDirectory, "Runtime/Plugins/Windows/x86_64");
            foreach (string file in Directory.GetFiles(directory, "*.dll"))
            {
                string path = root + "/" + Path.GetFileName(file);
                AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceSynchronousImport | ImportAssetOptions.ForceUpdate);
                if (AssetImporter.GetAtPath(path) is not PluginImporter importer)
                    throw new InvalidOperationException($"Native Windows plug-in was not imported: {path}");
    ...
    internal static class OnnxRuntimeProviderPluginFilter
    {
            ...
            IReadOnlyCollection<string> defines)
        {
            ...
            if (windows || linux) providers.Add(OnnxExecutionProvider.WebGpu);
    ...
    internal sealed class OnnxRuntimeProviderBuildValidator : IPreprocessBuildWithReport, IPostprocessBuildWithReport
    {
        ...
        public void OnPostprocessBuild(BuildReport report)
        {
            ...
            {
                runtimeVersion = typeof(Microsoft.ML.OnnxRuntime.OrtEnv).Assembly.GetName().Version.ToString(),
            ...
            string platformFolder, HashSet<string> defines, bool windows, bool linux, bool android)
        {
            ...
            if (windows || linux) RequirePlugin(root, "webgpu");
            if (windows)
            {
                RequirePlugin(root, "dxcompiler");
                RequirePlugin(root, "dxil");
            }
        }
        ...
        private static bool IsProviderPayload(string fileName)
        {
            ...
            return name.Contains("onnxruntime") || name.Contains("dxcompiler") || name.Contains("dxil") || name.Contains("cuda") ||
